...
import { getThemeStyleTag, renderJsonToHtml } from 'json-html-kit';
...
function bindJsonHtmlDemo(): void {
  ...
  const update = (): void => {
    ...
    output.innerHTML = value.ok
      ? `${getThemeStyleTag(theme.value as 'clean' | 'slate' | 'paper')}${renderJsonToHtml(value.data, {
          theme: theme.value as 'clean' | 'slate' | 'paper',
          tableMode: 'auto',
          collapseDepth: 3
        })}`
